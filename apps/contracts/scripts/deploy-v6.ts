...
import { parseV6Config } from "./v6-config.js";
...
async function main() {
// Read-only unless V6_BROADCAST=1 is explicitly set. No automatic request, mint or prize transaction.
const broadcast = process.env.V6_BROADCAST === "1";
if (process.env.V6_BROADCAST && !["0", "1"].includes(process.env.V6_BROADCAST)) throw new DeploymentError("V6_BROADCAST must be 0 or 1.");
const path = process.env.V6_CONFIG_PATH;
if (!path) throw new DeploymentError("Set V6_CONFIG_PATH to reviewed V6 terms; example zero owners are intentionally invalid.");
...
const ceiling = feeCeiling(process.env.V6_MAX_FEE_PER_GAS_WEI);
if (broadcast && !process.env.V6_JOURNAL_PATH) throw new DeploymentError("Broadcast requires an explicit V6_JOURNAL_PATH. Reuse this path for every resume.");
const store = broadcast ? await openJournal(process.env.V6_JOURNAL_PATH!) : null;
...
const terms = parseV6Config(raw, chainId, configTimestamp);
if (!process.env.V6_AFFILIATE_ELIGIBILITY_ADDRESS) throw new DeploymentError("Set the finalized V6_AFFILIATE_ELIGIBILITY_ADDRESS before deployment.");
const affiliateEligibility = ethers.getAddress(process.env.V6_AFFILIATE_ELIGIBILITY_ADDRESS);
const eligibilityPin = process.env[`AFFILIATE_ELIGIBILITY_CODEHASH_${chainId}`];
if (affiliateEligibility === ethers.ZeroAddress || process.env[`AFFILIATE_ELIGIBILITY_ADDRESS_${chainId}`]?.toLowerCase() !== affiliateEligibility.toLowerCase()
...
const from = signer?.address ?? ethers.getAddress(process.env.V6_PREFLIGHT_FROM ?? terms.config.initialOwner);
...
const factoryArtifact = await artifacts.readArtifact("ManekinekoFactoryV6");
const roundArtifact = await artifacts.readArtifact("ManekinekoRoundV6");
const rendererArtifact = await artifacts.readArtifact("ManekinekoRendererV6");
const deployerArtifact = await artifacts.readArtifact("ManekinekoRoundDeployerV6");
const eligibilityArtifact = await artifacts.readArtifact("ManekinekoAffiliateEligibility");
...
if (await eligibility.ELIGIBILITY_VERSION({ blockTag: block.number }) !== "affiliate-eligibility-v1") throw new DeploymentError("Unsupported holder-eligibility registry version.");
...
if (process.env.FACTORY_ADDRESS) {
  ...
  if (!matchesRuntime(await ethers.provider.getCode(address, block.number), factoryArtifact)) {
    throw new DeploymentError("Existing factory runtime differs from this V6 build.");
  }
  ...
  if (!matchesRuntime(await ethers.provider.getCode(renderer, block.number), rendererArtifact)) throw new DeploymentError("Existing factory renderer differs from this V6 build.");
  const deployerAddress = await existingFactory.deployer({ blockTag: block.number });
  if (!matchesRuntime(await ethers.provider.getCode(deployerAddress, block.number), deployerArtifact)) throw new DeploymentError("Existing factory deployer differs from this V6 build.");
  ...
  if (roundId > 1n) {
    const previous = await ethers.getContractAt("ManekinekoRoundV6", await existingFactory.rounds(roundId - 1n, { blockTag: block.number }));
    if (!await previous.readyForNextRound({ blockTag: block.number })) throw new DeploymentError("Previous V6 round has not delivered its prize.");
  }
}
...
const report = {
  ...
  enrollmentNote: "After canonical eligibility registration, enrollment stays available until the owner activates minting. Automated admission needs a separately configured web signer and bot checks. No mint or enrollment transaction is broadcast by this script.",
...
if (broadcast) {
  ...
  try {
    ...
    if (!matchesRuntime(deployedFactoryCode, factoryArtifact) ||
      ...
      throw new DeploymentError("Deployed factory, immutable renderer or deployer does not match the local V6 build.");
    }
    ...
    const round = await ethers.getContractAt("ManekinekoRoundV6", roundAddress, signer);
    journal.round = roundAddress;
    journal.subscriptionId = String(await round.subscriptionId(roundBlock));
    await save();
    if (!matchesRuntime(await ethers.provider.getCode(roundAddress, roundReceipt.blockNumber), roundArtifact)) throw new DeploymentError("Deployed round runtime does not match the local V6 build.");
    if (await round.renderer(roundBlock) !== journal.renderer || await round.maxAffiliateSlots(roundBlock) !== config.maxAffiliateSlots ||
        await round.enrollmentSigner(roundBlock) !== config.enrollmentSigner || await round.affiliateEligibility(roundBlock) !== affiliateEligibility || await round.CONTRACT_VERSION(roundBlock) !== "affiliate-v6" || await round.ALGORITHM_VERSION(roundBlock) !== "unique-rank-v3") {
...
main().catch(error => {
  console.error(error instanceof DeploymentError ? error.message : "V6 deployment stopped; sensitive provider diagnostics were withheld. Reconcile V6_JOURNAL_PATH before resuming.");
  process.exitCode = 1;
});
