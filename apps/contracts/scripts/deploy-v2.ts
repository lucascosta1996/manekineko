...
// Read-only unless V2_BROADCAST=1 is explicitly set. No automatic request, mint or prize transaction.
const broadcast = process.env.V2_BROADCAST === "1";
if (process.env.V2_BROADCAST && !["0", "1"].includes(process.env.V2_BROADCAST)) throw new Error("V2_BROADCAST must be 0 or 1.");
const path = process.env.V2_CONFIG_PATH;
if (!path) throw new Error("Set V2_CONFIG_PATH to reviewed V2 terms; example zero owners are intentionally invalid.");
...
const terms = parseV2Config(raw, chainId, BigInt(block.timestamp));
...
const from = signer?.address ?? ethers.getAddress(process.env.V2_PREFLIGHT_FROM ?? terms.config.initialOwner);
...
const factoryArtifact = await artifacts.readArtifact("ManekinekoFactoryV2");
const roundArtifact = await artifacts.readArtifact("ManekinekoRoundV2");
for (const artifact of [factoryArtifact, roundArtifact]) {
...
if (process.env.FACTORY_ADDRESS) {
  ...
  if ((await ethers.provider.getCode(address, block.number)).toLowerCase() !== factoryArtifact.deployedBytecode.toLowerCase()) {
    throw new Error("Existing factory runtime differs from this V2 build.");
  }
  existingFactory = await ethers.getContractAt("ManekinekoFactoryV2", address, signer ?? ethers.provider);
  ...
  if (roundId > 1n) {
    const previous = await ethers.getContractAt("ManekinekoRoundV2", await existingFactory.rounds(roundId - 1n, { blockTag: block.number }));
    if (!await previous.readyForNextRound({ blockTag: block.number })) throw new Error("Previous V2 round has not delivered its prize.");
  }
}
...
const roundBuilder = new ethers.ContractFactory(roundArtifact.abi, roundArtifact.bytecode, ethers.provider);
...
const roundGas = existingFactory
  ...
  : await ethers.provider.estimateGas({ ...await roundBuilder.getDeployTransaction(config), from });
...
const report = {
  ...
  coordinatorChecks, feePerGas, factoryGas, roundGas, estimatedTransactionBudgetWei, senderBalanceWei: balance,
...
if (broadcast) {
  ...
  const file = new URL(`${chainId}-v2-${Date.now()}-${roundId}.json`, directory);
  ...
  try {
    ...
    if (!factory) {
      ...
      factory = await ethers.getContractAt("ManekinekoFactoryV2", receipt.contractAddress, signer);
    }
    ...
    const round = await ethers.getContractAt("ManekinekoRoundV2", roundAddress, signer);
    ...
    journal.state = terms.activateSale ? "sale-active" : "funded-awaiting-owner-activation";
    await save();
    console.log(json({ journal: file.pathname, factory: journal.factory, round: roundAddress, subscriptionId: journal.subscriptionId, state: journal.state }));
  } catch (error) {
