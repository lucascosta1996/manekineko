...
import { parseV9Config } from "./v9-config.js";
...
async function main() {
// Read-only unless V9_BROADCAST=1 is explicitly set. No automatic request, mint or prize transaction.
const broadcast = process.env.V9_BROADCAST === "1";
if (process.env.V9_BROADCAST && !["0", "1"].includes(process.env.V9_BROADCAST)) throw new DeploymentError("V9_BROADCAST must be 0 or 1.");
const path = process.env.V9_CONFIG_PATH;
if (!path) throw new DeploymentError("Set V9_CONFIG_PATH to reviewed V9 terms; example zero owners are intentionally invalid.");
...
const ceiling = feeCeiling(process.env.V9_MAX_FEE_PER_GAS_WEI);
if (broadcast && !process.env.V9_JOURNAL_PATH) throw new DeploymentError("Broadcast requires an explicit V9_JOURNAL_PATH. Reuse this path for every resume.");
const store = broadcast ? await openJournal(process.env.V9_JOURNAL_PATH!) : null;
...
const terms = parseV9Config(raw, chainId, configTimestamp);
if (!process.env.V9_AFFILIATE_ELIGIBILITY_ADDRESS) throw new DeploymentError("Set the finalized V9_AFFILIATE_ELIGIBILITY_ADDRESS before deployment.");
const affiliateEligibility = ethers.getAddress(process.env.V9_AFFILIATE_ELIGIBILITY_ADDRESS);
const eligibilityPin = process.env[`AFFILIATE_ELIGIBILITY_V4_CODEHASH_${chainId}`];
if (affiliateEligibility === ethers.ZeroAddress || process.env[`AFFILIATE_ELIGIBILITY_V4_ADDRESS_${chainId}`]?.toLowerCase() !== affiliateEligibility.toLowerCase()
...
const from = signer?.address ?? ethers.getAddress(process.env.V9_PREFLIGHT_FROM ?? terms.config.initialOwner);
...
const factoryArtifact = await artifacts.readArtifact("ManekinekoFactoryV9");
const roundArtifact = await artifacts.readArtifact("ManekinekoRoundV9");
const rendererArtifact = await artifacts.readArtifact("ManekinekoRendererV9");
const deployerArtifact = await artifacts.readArtifact("ManekinekoRoundDeployerV9");
async function verifyTemplateParts(deployerAddress: string, blockNumber: number) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (code.toLowerCase() !== ethers.hexlify(ethers.concat(["0x00", expected[i]])).toLowerCase())
      throw new DeploymentError("V9 immutable creation-code part differs from the reviewed round build.");
  }
}
const eligibilityArtifact = await artifacts.readArtifact("ManekinekoAffiliateEligibilityV4");
...
if (await eligibility.ELIGIBILITY_VERSION({ blockTag: block.number }) !== "affiliate-eligibility-v4") throw new DeploymentError("Unsupported holder-eligibility registry version.");
...
if (process.env.FACTORY_ADDRESS) {
  ...
  if (!matchesRuntime(await ethers.provider.getCode(address, block.number), factoryArtifact)) {
    throw new DeploymentError("Existing factory runtime differs from this V9 build.");
  }
  ...
  if (!matchesRuntime(await ethers.provider.getCode(renderer, block.number), rendererArtifact)) throw new DeploymentError("Existing factory renderer differs from this V9 build.");
  const deployerAddress = await existingFactory.deployer({ blockTag: block.number });
  if (!matchesRuntime(await ethers.provider.getCode(deployerAddress, block.number), deployerArtifact)) throw new DeploymentError("Existing factory deployer differs from this V9 build.");
  ...
  if (roundId > 1n) {
    const previous = await ethers.getContractAt("ManekinekoRoundV9", await existingFactory.rounds(roundId - 1n, { blockTag: block.number }));
    if (!await previous.readyForNextRound({ blockTag: block.number })) throw new DeploymentError("Previous V9 round is not revealed with funded award liabilities.");
  }
}
...
if (broadcast) {
  ...
  try {
    ...
    if (!matchesRuntime(deployedFactoryCode, factoryArtifact) ||
      ...
      throw new DeploymentError("Deployed factory, immutable renderer or deployer does not match the local V9 build.");
    }
    ...
    const round = await ethers.getContractAt("ManekinekoRoundV9", roundAddress, signer);
    journal.round = roundAddress;
    journal.subscriptionId = String(await round.subscriptionId(roundBlock));
    await save();
    if (!matchesRuntime(await ethers.provider.getCode(roundAddress, roundReceipt.blockNumber), roundArtifact)) throw new DeploymentError("Deployed round runtime does not match the local V9 build.");
    if (await round.renderer(roundBlock) !== journal.renderer || await round.maxAffiliateSlots(roundBlock) !== config.maxAffiliateSlots ||
        await round.enrollmentSigner(roundBlock) !== config.enrollmentSigner || await round.affiliateEligibility(roundBlock) !== affiliateEligibility || await round.CONTRACT_VERSION(roundBlock) !== "affiliate-v9" || await round.ALGORITHM_VERSION(roundBlock) !== "unique-rank-v5") {
...
main().catch(error => {
  console.error(error instanceof DeploymentError ? error.message : "V9 deployment stopped; sensitive provider diagnostics were withheld. Reconcile V9_JOURNAL_PATH before resuming.");
  process.exitCode = 1;
});
