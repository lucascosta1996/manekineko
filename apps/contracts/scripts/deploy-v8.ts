...
import { parseV8Config } from "./v8-config.js";
...
async function main() {
// Read-only unless V8_BROADCAST=1 is explicitly set. No automatic request, mint or prize transaction.
const broadcast = process.env.V8_BROADCAST === "1";
if (process.env.V8_BROADCAST && !["0", "1"].includes(process.env.V8_BROADCAST)) throw new DeploymentError("V8_BROADCAST must be 0 or 1.");
const path = process.env.V8_CONFIG_PATH;
if (!path) throw new DeploymentError("Set V8_CONFIG_PATH to reviewed V8 terms; example zero owners are intentionally invalid.");
...
const ceiling = feeCeiling(process.env.V8_MAX_FEE_PER_GAS_WEI);
if (broadcast && !process.env.V8_JOURNAL_PATH) throw new DeploymentError("Broadcast requires an explicit V8_JOURNAL_PATH. Reuse this path for every resume.");
const store = broadcast ? await openJournal(process.env.V8_JOURNAL_PATH!) : null;
...
const terms = parseV8Config(raw, chainId, configTimestamp);
if (!process.env.V8_AFFILIATE_ELIGIBILITY_ADDRESS) throw new DeploymentError("Set the finalized V8_AFFILIATE_ELIGIBILITY_ADDRESS before deployment.");
const affiliateEligibility = ethers.getAddress(process.env.V8_AFFILIATE_ELIGIBILITY_ADDRESS);
const eligibilityPin = process.env[`AFFILIATE_ELIGIBILITY_V3_CODEHASH_${chainId}`];
if (affiliateEligibility === ethers.ZeroAddress || process.env[`AFFILIATE_ELIGIBILITY_V3_ADDRESS_${chainId}`]?.toLowerCase() !== affiliateEligibility.toLowerCase()
...
const from = signer?.address ?? ethers.getAddress(process.env.V8_PREFLIGHT_FROM ?? terms.config.initialOwner);
...
const factoryArtifact = await artifacts.readArtifact("ManekinekoFactoryV8");
const roundArtifact = await artifacts.readArtifact("ManekinekoRoundV8");
const rendererArtifact = await artifacts.readArtifact("ManekinekoRendererV8");
const deployerArtifact = await artifacts.readArtifact("ManekinekoRoundDeployerV8");
async function verifyTemplateParts(deployerAddress: string, blockNumber: number) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (code.toLowerCase() !== ethers.hexlify(ethers.concat(["0x00", expected[i]])).toLowerCase())
      throw new DeploymentError("V8 immutable creation-code part differs from the reviewed round build.");
  }
}
const eligibilityArtifact = await artifacts.readArtifact("ManekinekoAffiliateEligibilityV3");
...
if (await eligibility.ELIGIBILITY_VERSION({ blockTag: block.number }) !== "affiliate-eligibility-v3") throw new DeploymentError("Unsupported holder-eligibility registry version.");
...
if (process.env.FACTORY_ADDRESS) {
  ...
  if (!matchesRuntime(await ethers.provider.getCode(address, block.number), factoryArtifact)) {
    throw new DeploymentError("Existing factory runtime differs from this V8 build.");
  }
  ...
  if (!matchesRuntime(await ethers.provider.getCode(renderer, block.number), rendererArtifact)) throw new DeploymentError("Existing factory renderer differs from this V8 build.");
  const deployerAddress = await existingFactory.deployer({ blockTag: block.number });
  if (!matchesRuntime(await ethers.provider.getCode(deployerAddress, block.number), deployerArtifact)) throw new DeploymentError("Existing factory deployer differs from this V8 build.");
  ...
  if (roundId > 1n) {
    const previous = await ethers.getContractAt("ManekinekoRoundV8", await existingFactory.rounds(roundId - 1n, { blockTag: block.number }));
    if (!await previous.readyForNextRound({ blockTag: block.number })) throw new DeploymentError("Previous V8 round is not revealed with funded award liabilities.");
  }
}
...
if (broadcast) {
  ...
  try {
    ...
    if (!matchesRuntime(deployedFactoryCode, factoryArtifact) ||
      ...
      throw new DeploymentError("Deployed factory, immutable renderer or deployer does not match the local V8 build.");
    }
    ...
    const round = await ethers.getContractAt("ManekinekoRoundV8", roundAddress, signer);
    journal.round = roundAddress;
    journal.subscriptionId = String(await round.subscriptionId(roundBlock));
    await save();
    if (!matchesRuntime(await ethers.provider.getCode(roundAddress, roundReceipt.blockNumber), roundArtifact)) throw new DeploymentError("Deployed round runtime does not match the local V8 build.");
    if (await round.renderer(roundBlock) !== journal.renderer || await round.maxAffiliateSlots(roundBlock) !== config.maxAffiliateSlots ||
        await round.enrollmentSigner(roundBlock) !== config.enrollmentSigner || await round.affiliateEligibility(roundBlock) !== affiliateEligibility || await round.CONTRACT_VERSION(roundBlock) !== "affiliate-v8" || await round.ALGORITHM_VERSION(roundBlock) !== "unique-rank-v5") {
...
main().catch(error => {
  console.error(error instanceof DeploymentError ? error.message : "V8 deployment stopped; sensitive provider diagnostics were withheld. Reconcile V8_JOURNAL_PATH before resuming.");
  process.exitCode = 1;
});
