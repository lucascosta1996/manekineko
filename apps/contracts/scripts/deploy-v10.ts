...
import { parseV10Config } from "./v10-config.js";
import { AbiCoder, Wallet, id, keccak256, getCreateAddress } from "ethers";
...
async function main() {
// Read-only unless V10_BROADCAST=1 is explicitly set. No automatic request, mint or prize transaction.
const broadcast = process.env.V10_BROADCAST === "1";
if (process.env.V10_BROADCAST && !["0", "1"].includes(process.env.V10_BROADCAST)) throw new DeploymentError("V10_BROADCAST must be 0 or 1.");
const path = process.env.V10_CONFIG_PATH;
if (!path) throw new DeploymentError("Set V10_CONFIG_PATH to reviewed V10 terms; example zero owners are intentionally invalid.");
...
const ceiling = feeCeiling(process.env.V10_MAX_FEE_PER_GAS_WEI);
if (broadcast && !process.env.V10_JOURNAL_PATH) throw new DeploymentError("Broadcast requires an explicit V10_JOURNAL_PATH. Reuse this path for every resume.");
const store = broadcast ? await openJournal(process.env.V10_JOURNAL_PATH!) : null;
...
const terms = parseV10Config(raw, chainId, configTimestamp);
if (!process.env.V10_AFFILIATE_ELIGIBILITY_ADDRESS) throw new DeploymentError("Set the finalized V10_AFFILIATE_ELIGIBILITY_ADDRESS before deployment.");
const affiliateEligibility = ethers.getAddress(process.env.V10_AFFILIATE_ELIGIBILITY_ADDRESS);
const eligibilityPin = process.env[`AFFILIATE_ELIGIBILITY_V5_CODEHASH_${chainId}`];
if (affiliateEligibility === ethers.ZeroAddress || process.env[`AFFILIATE_ELIGIBILITY_V5_ADDRESS_${chainId}`]?.toLowerCase() !== affiliateEligibility.toLowerCase()
  || !/^0x[0-9a-f]{64}$/i.test(eligibilityPin ?? "")) throw new DeploymentError("The reviewed holder-eligibility address must match both canonical network pins.");
...
const from = signer?.address ?? ethers.getAddress(process.env.V10_PREFLIGHT_FROM ?? terms.config.initialOwner);
...
const factoryArtifact = await artifacts.readArtifact("ManekinekoFactoryV10");
const roundArtifact = await artifacts.readArtifact("ManekinekoRoundV10");
const rendererArtifact = await artifacts.readArtifact("ManekinekoRendererV10");
const deployerArtifact = await artifacts.readArtifact("ManekinekoRoundDeployerV10");
async function verifyTemplateParts(deployerAddress: string, blockNumber: number) {
  const template = new ethers.Contract(deployerAddress, deployerArtifact.abi, ethers.provider);
  const bytecode = ethers.getBytes(roundArtifact.bytecode);
  const split = Math.floor(bytecode.length / 2);
  const parts = [await template.codePart1({ blockTag: blockNumber }), await template.codePart2({ blockTag: blockNumber })];
  const expected = [bytecode.slice(0, split), bytecode.slice(split)];
  for (let i = 0; i < 2; i++) {
    const code = await ethers.provider.getCode(parts[i], blockNumber);
    if (code.toLowerCase() !== ethers.hexlify(ethers.concat(["0x00", expected[i]])).toLowerCase())
      throw new DeploymentError("V10 immutable creation-code part differs from the reviewed round build.");
  }
}
const eligibilityArtifact = await artifacts.readArtifact("ManekinekoAffiliateEligibilityV5");
const eligibilityCode = await ethers.provider.getCode(affiliateEligibility, block.number);
if (!matchesRuntime(eligibilityCode, eligibilityArtifact) || ethers.keccak256(eligibilityCode).toLowerCase() !== eligibilityPin!.toLowerCase()) throw new DeploymentError("Canonical holder-eligibility registry differs from the reviewed build or runtime pin.");
const eligibility = new ethers.Contract(affiliateEligibility, eligibilityArtifact.abi, ethers.provider);
if (await eligibility.ELIGIBILITY_VERSION({ blockTag: block.number }) !== "affiliate-eligibility-v5") throw new DeploymentError("Unsupported holder-eligibility registry version.");
...
const inputHash = configFingerprint({ terms: raw, affiliateEligibility, eligibilityCodeHash: eligibilityPin!.toLowerCase() });
if (previous) {
...
if (process.env.FACTORY_ADDRESS) {
  ...
  if (!matchesRuntime(await ethers.provider.getCode(address, block.number), factoryArtifact)) {
    throw new DeploymentError("Existing factory runtime differs from this V10 build.");
  }
  ...
  if (!matchesRuntime(await ethers.provider.getCode(renderer, block.number), rendererArtifact)) throw new DeploymentError("Existing factory renderer differs from this V10 build.");
  const deployerAddress = await existingFactory.deployer({ blockTag: block.number });
  if (!matchesRuntime(await ethers.provider.getCode(deployerAddress, block.number), deployerArtifact)) throw new DeploymentError("Existing factory deployer differs from this V10 build.");
  const deployer = new ethers.Contract(deployerAddress, deployerArtifact.abi, ethers.provider);
  await verifyTemplateParts(deployerAddress, block.number);
  ...
  const seasonCount = await existingFactory.seasonCollectionCount(terms.config.seasonId, { blockTag: block.number });
  if (!previous && seasonCount >= 10n) throw new DeploymentError("This season already contains ten collections. Start a new season.");
  const storedNameHash = await existingFactory.seasonNameHash(terms.config.seasonId, { blockTag: block.number });
  if (seasonCount > 0n && storedNameHash !== ethers.id(terms.config.seasonName)) throw new DeploymentError("Season name does not match the existing on-chain season.");
  if (roundId > 1n) {
    const previous = await ethers.getContractAt("ManekinekoRoundV10", await existingFactory.rounds(roundId - 1n, { blockTag: block.number }));
    if (!await previous.readyForNextRound({ blockTag: block.number })) throw new DeploymentError("Previous V10 round is not revealed with funded award liabilities.");
  }
}
const config = { ...terms.config, roundId, affiliateEligibility };
...
const report = {
  ...
  winnerCount: config.winnerCount, minAffiliateReferrals: config.minAffiliateReferrals, affiliatePayoutCapBps: config.affiliatePayoutCapBps, saleStartAt: config.saleStartAt,
  accountingNote: "At sellout qualified affiliates share equally, capped by the configured percentage of the lowest qualifying referral revenue. Unallocated funds are a separately withdrawable growth reserve. All equal holder prizes remain reserved until individually claimed.",
  enrollmentNote: "After canonical eligibility registration, enrollment stays available until the immutable saleStartAt; activation and minting cannot occur before that time. Automated admission needs a separately configured web signer and bot checks. No mint or enrollment transaction is broadcast by this script.",
  affiliateEligibility, eligibilityCodeHash: eligibilityPin!.toLowerCase(),
  eligibilityNote: "The canonical registry owner must approve this factory and register the collection before enrollment or activation. Only the first canonical collection has open enrollment.",
};
...
if (broadcast) {
  ...
  try {
    ...
    if (!matchesRuntime(deployedFactoryCode, factoryArtifact) ||
      ...
      throw new DeploymentError("Deployed factory, immutable renderer or deployer does not match the local V10 build.");
    }
    await verifyTemplateParts(String(journal.deployer), factoryBlock);
    ...
    const round = await ethers.getContractAt("ManekinekoRoundV10", roundAddress, signer);
    journal.round = roundAddress;
    journal.subscriptionId = String(await round.subscriptionId(roundBlock));
    await save();
    if (!matchesRuntime(await ethers.provider.getCode(roundAddress, roundReceipt.blockNumber), roundArtifact)) throw new DeploymentError("Deployed round runtime does not match the local V10 build.");
    if (await round.renderer(roundBlock) !== journal.renderer || await round.maxAffiliateSlots(roundBlock) !== config.maxAffiliateSlots ||
        await round.enrollmentSigner(roundBlock) !== config.enrollmentSigner || await round.affiliateEligibility(roundBlock) !== affiliateEligibility || await round.CONTRACT_VERSION(roundBlock) !== "affiliate-v10" || await round.ALGORITHM_VERSION(roundBlock) !== "unique-rank-v6") {
    ...
    if (await round.prizeBps(roundBlock) !== config.prizeBps || await round.affiliatePoolBps(roundBlock) !== config.affiliatePoolBps ||
        await round.winnerCount(roundBlock) !== config.winnerCount || await round.minAffiliateReferrals(roundBlock) !== config.minAffiliateReferrals ||
        await round.affiliatePayoutCapBps(roundBlock) !== config.affiliatePayoutCapBps || await round.saleStartAt(roundBlock) !== config.saleStartAt ||
    ...
    const expectedCombinationKey = keccak256(AbiCoder.defaultAbiCoder().encode(
      ["bytes32", "uint256", "address", "uint256", "bytes32", "uint256"],
      [id("MANEKINEKO_PERMANENT_COMBINATION_V1"), chainId, roundAddress, config.roundId, config.seasonId, config.maxSupply],
    ));
    if (await round.MAX_MINTS_PER_WALLET(roundBlock) !== 20n || await round.combinationKey(roundBlock) !== expectedCombinationKey) {
      throw new DeploymentError("Post-deployment wallet cap or permanent combination key differs.");
    }
    if (await round.seasonId(roundBlock) !== config.seasonId || await round.seasonName(roundBlock) !== config.seasonName ||
        await round.collectionColor(roundBlock) !== config.collectionColor || await round.textColor(roundBlock) !== config.textColor ||
        await factory.seasonNameHash(config.seasonId, roundBlock) !== ethers.id(config.seasonName) ||
        await factory.seasonCollectionCount(config.seasonId, roundBlock) > 10n) {
      throw new DeploymentError("Post-deployment season and artwork checks failed.");
    }
    ...
    if (await round.saleActivated({ blockTag: fundingReceipt.blockNumber })) throw new DeploymentError("Unexpected sale activation. Deployment must leave minting inactive for eligibility setup and enrollment.");
    ...
    const registration = await eligibility.collections(roundAddress);
    journal.state = registration.sequence > 0n ? "funded-enrollment-open" : "funded-awaiting-affiliate-registration";
...
main().catch(error => {
  console.error(error instanceof DeploymentError ? error.message : "V10 deployment stopped; sensitive provider diagnostics were withheld. Reconcile V10_JOURNAL_PATH before resuming.");
  process.exitCode = 1;
});
