...
async function fixture(){
 ...
 const config={name:"Six equal prizes",symbol:"NEKO10",seasonId:ethers.id("Season V10"),seasonName:"Season V10",collectionColor:"#330000",textColor:"#FFFFFF",roundId:1n,maxSupply:1000n,mintPrice:PRICE,mintDeadline:BigInt(await time.latest())+86400n,saleStartAt:BigInt(await time.latest())+3600n,initialOwner:owner.address,vrfCoordinator:await coordinator.getAddress(),keyHash:ethers.id("mock-key"),requestConfirmations:64,callbackGasLimit:200000,maxAffiliateSlots:10n,enrollmentSigner:enrollmentSigner.address,prizeBps:6000n,winnerCount:6n,affiliatePoolBps:2000n,minAffiliateReferrals:100n,affiliatePayoutCapBps:3000n,affiliateEligibility:await eligibility.getAddress()};
 await factory.createRound(config);const round=await ethers.getContractAt("ManekinekoRoundV10",await factory.rounds(1));await eligibility.registerCollection(await factory.getAddress(),1);await round.fundRandomness({value:await coordinator.MOCK_FEE()});
...
async function enroll(c:C){
 const nonce=ethers.id("v10-enroll"),deadline=c.config.saleStartAt-1n;
...
describe('V10 cumulative 20-ticket wallet allowance', function () {
 this.timeout(180000);
 ...
 it('sells 1000 tickets to 50 wallets, settles six one-ETH prizes and preserves the affiliate pool',async()=>{
  ...
  const credits=await ethers.deployContract('ManekinekoWinnerCreditsV6',[c.owner.address,ethers.ZeroHash,ethers.ZeroAddress]);
  ...
  const rankHarness=await ethers.deployContract('MultiAwardRankHarness'),ranks=await rankHarness.ranks(1000,[...ids,0n,0n,0n,0n],6);
  expect([...ranks].sort((a,b)=>Number(a-b))).deep.eq(Array.from({length:1000},(_,i)=>BigInt(i+1)));
  for(let rank=1;rank<=6;rank++){
   ...
   const [numbers,,score]=await c.round.combination(ids[rank-1]);expect(score).eq(BigInt(1001-rank));expect(await c.round.score(ids[rank-1])).eq(score);expect(await c.round.scoreCombination([...numbers])).eq(score);
  ...
  const next=await ethers.getContractAt('ManekinekoRoundV10',await c.factory.rounds(2));await next.fundRandomness({value:await c.coordinator.MOCK_FEE()});await credits.fundCollection(await next.getAddress(),{value:PRICE*6n});
  ...
  for(const component of ['RoundV10' ,'FactoryV10','RendererV10','RoundDeployerV10','AffiliateEligibilityV5','WinnerCreditsV6']){const a=await artifacts.readArtifact(`Manekineko${component}`);expect((a.deployedBytecode.length-2)/2,component).at.most(24576);expect((a.bytecode.length-2)/2,component).at.most(49152);}
 });
});
