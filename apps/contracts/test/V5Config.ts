...
describe("V5 immutable pool configuration", function () {
  it("keeps the pool independent of position count", function () {
    for (const maxAffiliateSlots of ["1", "10", "20", "100"]) {
      const result = parseV5Config({ ...terms, maxAffiliateSlots }, 1n, 1000n);
  ...
  it("rejects ambiguous, oversubscribed, noncanonical and incompatible V4 terms", function () {
    ...
    ]) expect(() => parseV5Config({ ...terms, ...override }, 1n, 1000n)).to.throw();
  });
  it("accepts zero-pool and full-pool boundaries and empty compatibility rate arrays", function () {
    expect(parseV5Config({ ...terms, prizeBps: "0", affiliatePoolBps: "10000", affiliateRatesBps: [] }, 1n, 1000n).config.affiliatePoolBps).to.equal(10000n);
    expect(parseV5Config({ ...terms, prizeBps: "10000", affiliatePoolBps: "0" }, 1n, 1000n).config.affiliatePoolBps).to.equal(0n);
  });
});
