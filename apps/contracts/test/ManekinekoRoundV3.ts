...
// V3 preserves the V2 lifecycle invariants with a separate immutable deployment.
describe("ManekinekoRoundV3 VRF lifecycle regression", function () {
  ...
  it("rejects invalid constructor configuration and disables ownership renunciation", async function () {
    ...
    for (const override of invalid) {
      await expect(ethers.deployContract("ManekinekoRoundV3", [{ ...config, ...override }, await round.renderer()]))
        .to.be.revertedWithCustomError(round, "InvalidConfig");
    }
    await expect(ethers.deployContract("ManekinekoRoundV3", [{ ...config, initialOwner: ZERO }, await round.renderer()]))
  ...
  it("gates factory rollover on actual prize delivery and gives each round its own subscription", async function () {
    ...
    const factory = await ethers.deployContract("ManekinekoFactoryV3", [owner.address]);
    ...
    const roundFactory = await ethers.getContractFactory("ManekinekoRoundV3");
    ...
    const factoryDeployment = await (await ethers.getContractFactory("ManekinekoFactoryV3")).getDeployTransaction(owner.address);
  ...
  for (const supply of [1, 1_000, 2_000]) {
    it(`gives all ${supply} referred NFTs distinct scores, one winner and exactly 1% protected commissions`, async function () {
      ...
      const signature = await operator.signTypedData({
        name: "ManekinekoAffiliateEnrollment", version: "1", chainId: 31337,
        verifyingContract: await round.getAddress(),
      }, { Enrollment: [
        { name: "applicant", type: "address" }, { name: "nonce", type: "bytes32" }, { name: "deadline", type: "uint256" },
      ] }, { applicant: other.address, nonce, deadline });
      await round.connect(other).enrollAffiliate(other.address, nonce, deadline, signature);
