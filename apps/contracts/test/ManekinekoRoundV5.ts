...
// V5 preserves the V2 lifecycle invariants with a separate immutable deployment.
describe("ManekinekoRoundV5 VRF lifecycle regression", function () {
  ...
  it("reserves a transferred winning ticket's claim for its holder, excluding the owner and NFT approvals", async function () {
    const context = await deployRound({ maxSupply: 20n });
    const { round, coordinator, owner, minter: previousHolder, holder, other: losingHolder, operator } = context;
    const [, , , , , approved] = await ethers.getSigners();
    await round.fundRandomness({ value: await coordinator.MOCK_FEE() });
    await round.activateSale();
    await round.connect(previousHolder).mint(previousHolder.address, 1, { value: PRICE });
    await round.connect(previousHolder).transferFrom(previousHolder.address, holder.address, 1);
    await round.connect(losingHolder).mint(losingHolder.address, 19, { value: PRICE * 19n });
    await round.connect(holder).approve(approved.address, 1);
    await round.connect(holder).setApprovalForAll(operator.address, true);

    // Select mock entropy solely to exercise the pre-sellout transfer of token 1.
    // A production caller cannot select or replace coordinator-authenticated entropy.
    const chainId = (await ethers.provider.getNetwork()).chainId;
    const domain = await round.DRAW_DOMAIN();
    const address = await round.getAddress();
    let word = 0n;
    while (true) {
      const encoded = ethers.AbiCoder.defaultAbiCoder().encode(
        ["bytes32", "uint256", "uint256", "address", "uint256", "uint256", "uint256"],
        [domain, word, chainId, address, context.config.roundId, 20n, 0n],
      );
      if (BigInt(ethers.keccak256(encoded)) % 20n === 19n) break;
      ++word;
    }
    await round.requestRandomness();
    await coordinator.fulfillRequest(await round.requestId(), word);
    ...
    expect(await round.balanceOf(losingHolder.address)).to.equal(19n);
    expect(await round.getApproved(1)).to.equal(approved.address);
    expect(await round.isApprovedForAll(holder.address, operator.address)).to.equal(true);
    expect(await round.withdrawableBalance()).to.equal(0n);
    await expect(round.withdraw(owner.address, 1n)).to.be.revertedWithCustomError(round, "InsufficientWithdrawableBalance");
    await expect(round.connect(holder).claimPrize(holder.address)).to.be.revertedWithCustomError(round, "ClaimNotAvailable");
    await time.increaseTo((await round.finalizedAt()) + (await round.PRIZE_CLAIM_DELAY()));
    for (const unauthorized of [owner, losingHolder, approved, operator, previousHolder]) {
      await expect(round.connect(unauthorized).claimPrize(unauthorized.address))
        .to.be.revertedWithCustomError(round, "NotTokenHolder");
    }
    await expect(round.connect(operator).transferFrom(holder.address, operator.address, 1))
      .to.be.revertedWithCustomError(round, "TransfersLocked");
    await expect(round.withdraw(owner.address, 1n)).to.be.revertedWithCustomError(round, "InsufficientWithdrawableBalance");
    const amount = await round.prizeAmount();
    await expect(round.connect(holder).claimPrize(holder.address))
      .to.changeEtherBalances(ethers, [round, holder], [-amount, amount]);
    expect(await round.prizeRecipient()).to.equal(holder.address);
    await expect(round.connect(holder).claimPrize(holder.address)).to.be.revertedWithCustomError(round, "InvalidPhase");
    await expect(round.distributePrize()).to.be.revertedWithCustomError(round, "InvalidPhase");
  });
  ...
  it("rejects invalid constructor configuration and disables ownership renunciation", async function () {
    ...
    for (const override of invalid) {
      await expect(ethers.deployContract("ManekinekoRoundV5", [{ ...config, ...override }, await round.renderer()]))
        .to.be.revertedWithCustomError(round, "InvalidConfig");
    }
    await expect(ethers.deployContract("ManekinekoRoundV5", [{ ...config, initialOwner: ZERO }, await round.renderer()]))
  ...
  it("gates factory rollover on actual prize delivery and gives each round its own subscription", async function () {
    ...
    const factory = await ethers.deployContract("ManekinekoFactoryV5", [owner.address]);
    ...
    const first = await ethers.getContractAt("ManekinekoRoundV5", await factory.rounds(1));
    ...
    const second = await ethers.getContractAt("ManekinekoRoundV5", await factory.rounds(2));
    ...
    const roundFactory = await ethers.getContractFactory("ManekinekoRoundV5");
    ...
    const factoryDeployment = await (await ethers.getContractFactory("ManekinekoFactoryV5")).getDeployTransaction(owner.address);
    expect(ethers.getBytes(factoryDeployment.data!).length).to.be.lessThanOrEqual(49_152);
  });

  for (const supply of [1, 20, 1_000, 2_000]) {
    it(`gives all ${supply} referred NFTs distinct scores, one winner and a protected 10% referral pool`, async function () {
      ...
      const signature = await operator.signTypedData({
        name: "ManekinekoAffiliateEnrollment", version: "3", chainId: 31337,
        verifyingContract: await round.getAddress(),
      }, { Enrollment: [
        { name: "applicant", type: "address" }, { name: "affiliateId", type: "uint256" }, { name: "poolBps", type: "uint256" }, { name: "nonce", type: "bytes32" }, { name: "deadline", type: "uint256" },
      ] }, { applicant: other.address, affiliateId: 1n, poolBps: 1000n, nonce, deadline });
      await round.connect(other).enrollAffiliate(other.address, 1, 1000, nonce, deadline, signature);
      ...
      for (let minted = 0, batch = 0; minted < supply; batch++) {
        const quantity = Math.min(20, Math.ceil(supply / 2), supply - minted);
      ...
      const commissions = PRICE * BigInt(supply) / 10n;
