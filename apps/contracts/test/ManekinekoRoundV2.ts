...
async function deployRound(overrides: Partial<RoundConfig> = {}) {
  ...
  const round = await ethers.deployContract("ManekinekoRoundV2", [config]);
...
describe("ManekinekoRoundV2 VRF lifecycle", function () {
  ...
  it("rejects invalid constructor configuration and disables ownership renunciation", async function () {
    ...
    for (const override of invalid) {
      await expect(ethers.deployContract("ManekinekoRoundV2", [{ ...config, ...override }]))
        .to.be.revertedWithCustomError(round, "InvalidConfig");
    }
    await expect(ethers.deployContract("ManekinekoRoundV2", [{ ...config, initialOwner: ZERO }]))
  ...
  it("gates factory rollover on actual prize delivery and gives each round its own subscription", async function () {
    ...
    const factory = await ethers.deployContract("ManekinekoFactoryV2", [owner.address]);
    ...
    const first = await ethers.getContractAt("ManekinekoRoundV2", await factory.rounds(1));
    ...
    const second = await ethers.getContractAt("ManekinekoRoundV2", await factory.rounds(2));
    ...
    const roundFactory = await ethers.getContractFactory("ManekinekoRoundV2");
    const roundDeployment = await roundFactory.getDeployTransaction(config);
    expect(ethers.getBytes(roundDeployment.data!).length).to.be.lessThanOrEqual(49_152);
    const factoryDeployment = await (await ethers.getContractFactory("ManekinekoFactoryV2")).getDeployTransaction(owner.address);
    expect(ethers.getBytes(factoryDeployment.data!).length).to.be.lessThanOrEqual(49_152);
  });

  for (const supply of [1, 1_000, 2_000]) {
    it(`gives all ${supply} NFTs distinct scores and exactly one maximum, then pays half once`, async function () {
      ...
      for (let minted = 0, batch = 0; minted < supply; batch++) {
        ...
        await round.connect(buyer).mint(buyer.address, quantity, { value: PRICE * BigInt(quantity) });
        minted += quantity;
      }
