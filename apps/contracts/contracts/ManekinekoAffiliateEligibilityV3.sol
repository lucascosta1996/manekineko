...
contract ManekinekoAffiliateEligibilityV3 is Ownable2Step, ReentrancyGuard {
    string public constant ELIGIBILITY_VERSION = "affiliate-eligibility-v3";
    ...
    /// @dev Existing V5/V6/V7 rounds can only be imported after successful completion and can never enroll through this gate.
    function registerCollection(address factory, uint256 id) external onlyOwner nonReentrant returns (address round) {
        ...
        bool sourceOnly = version == keccak256("affiliate-v5") || ((version == keccak256("affiliate-v6") || version == keccak256("affiliate-v7")) && _completed(candidate));
        if (candidate.roundId() != id || candidate.owner() == address(0) || candidate.maxSupply() == 0) revert InvalidCollection();
        if (sourceOnly) {
            if (keccak256(bytes(candidate.ALGORITHM_VERSION())) != (version == keccak256("affiliate-v5") ? keccak256("unique-rank-v2") : version == keccak256("affiliate-v6") ? keccak256("unique-rank-v3") : keccak256("unique-rank-v4")) || !_completed(candidate)) revert InvalidCollection();
        } else {
            bool supported = (version == keccak256("affiliate-v6") && keccak256(bytes(candidate.ALGORITHM_VERSION())) == keccak256("unique-rank-v3")) ||
                ...
                (version == keccak256("affiliate-v8") && keccak256(bytes(candidate.ALGORITHM_VERSION())) == keccak256("unique-rank-v5"));
            if (!supported ||
    ...
    function eligibilityStatus(address target, address applicant, address sourceCollection, uint256 sourceTokenId) public view returns (uint8) {
        ...
        bool v7 = version == keccak256("affiliate-v7") || version == keccak256("affiliate-v8");
    ...
    function _completed(IAffiliateEligibilityRound round) private view returns (bool) {
        ...
        if (version == keccak256("affiliate-v7") || version == keccak256("affiliate-v8")) return round.readyForNextRound();
