...
import {ManekinekoRoundV8} from "./ManekinekoRoundV8.sol";
import {ManekinekoRoundDeployerV8} from "./ManekinekoRoundDeployerV8.sol";
import {ManekinekoRendererV8} from "./ManekinekoRendererV8.sol";

/// @notice V8 configurable affiliate round registry. Failed/unsold rounds do not satisfy the successful-series rollover rule.
contract ManekinekoFactoryV8 is Ownable2Step {
    ManekinekoRendererV8 public immutable renderer;
    ManekinekoRoundDeployerV8 public immutable deployer;
    ...
    constructor(address initialOwner) Ownable(initialOwner) { renderer = new ManekinekoRendererV8(); deployer = new ManekinekoRoundDeployerV8(); }

    function createRound(ManekinekoRoundV8.Config calldata config) external onlyOwner returns (address round) {
        if (roundCount != 0 && !ManekinekoRoundV8(rounds[roundCount]).readyForNextRound()) revert PreviousRoundIncomplete();
