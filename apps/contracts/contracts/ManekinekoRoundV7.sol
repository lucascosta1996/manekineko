...
import {ManekinekoAffiliateEligibilityV2} from "./ManekinekoAffiliateEligibilityV2.sol";
...
/// @title A finite Ethereum NFT competition with one immutable VRF request and two distinct winning NFTs.
...
contract ManekinekoRoundV7 is ERC721, Ownable2Step, ReentrancyGuard {
    ...
    string public constant CONTRACT_VERSION = "affiliate-v7";
    ...
    string public constant ALGORITHM_VERSION = "unique-rank-v4";
    // Provider and score descriptions live in the shared on-chain renderer metadata.
    bytes32 public constant DRAW_DOMAIN = keccak256("MANEKINEKO_AWARD_RANK_V4");
    bytes32 private constant COMBINATION_DOMAIN = keccak256("MANEKINEKO_COMBINATION_V4");
    ...
    uint256 public immutable secondPrizeBps;
    ...
    ManekinekoAffiliateEligibilityV2 public immutable affiliateEligibility;
        ...
        ERC721(config.name, config.symbol) Ownable(config.initialOwner)
    {
        ...
        ManekinekoRendererV7(onchainRenderer).validateAppearance(config.seasonName, config.name, config.collectionColor, config.textColor);
        ManekinekoRendererV7(onchainRenderer).validateTerms(
        ...
        if (config.prizeBps == 0 || config.secondPrizeBps == 0 || config.secondPrizeBps >= config.prizeBps ||
            config.secondPrizeBps > config.prizeBps - config.secondPrizeBps ||
            config.maxSupply < 2 ||
        ...
        secondPrizeBps = config.secondPrizeBps;
        ...
        affiliateEligibility = ManekinekoAffiliateEligibilityV2(config.affiliateEligibility);
        renderer = ManekinekoRendererV7(onchainRenderer);
    ...
    /// @notice Explicit treasury withdrawal; never withdraws any affiliate entitlement or either NFT prize.
    ...
    function finalizeDraw(uint256 attempts) external nonReentrant {
        ...
        for (uint256 i; i < attempts; ++i) {
            ...
            (bool accepted, uint256 first, uint256 second) = AwardRank.tryWinners(candidate, maxSupply, awardCount());
            ++counter;
            if (accepted) {
                drawCounter = counter;
                winningTokenId = first;
                winningTokenIds[1] = first;
                if (secondPrizeBps != 0) winningTokenIds[2] = second;
                highestScore = maxSupply;
                finalizedAt = block.timestamp;
                revealed = true;
                emit WinnerDetermined(winningTokenId, highestScore, prizeAmountForRank(1));
                emit AwardDetermined(1, first, maxSupply, prizeAmountForRank(1));
                if (secondPrizeBps != 0) emit AwardDetermined(2, second, maxSupply - 1, prizeAmountForRank(2));
    ...
    /// A wallet owning both winning NFTs can claim both awards; each claim is independent.
    ...
    function _claimPrize(uint256 rank, address payable recipient) private {
        ...
        prizePaid = prizeClaimed[1] && (secondPrizeBps == 0 || prizeClaimed[2]);
        if (rank == 1) {
    ...
    function awardCount() public pure returns (uint256) { return 2; }
    function prizeAmountForRank(uint256 rank) public view returns (uint256) {
        if (rank == 0 || rank > awardCount()) revert InvalidQuantity();
        return (totalMintRevenue / BPS_DENOMINATOR) * (rank == 1 ? prizeBps - secondPrizeBps : secondPrizeBps);
    }
    ...
    /// @notice Settlement is final when both ranks are known and all unclaimed liabilities are funded.
    ...
    function combination(uint256 tokenId) public view returns (uint256[4] memory numbers, uint256 code, uint256 result) {
        ...
        result = AwardRank.rank(tokenId, maxSupply, winningTokenIds[1], winningTokenIds[2]);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return renderer.tokenURI(ManekinekoRendererV7.Appearance(seasonId, seasonName, name(), collectionColor(), textColor()), tokenId, refundsAvailable(), revealed, numbers, result, _awardRank(tokenId), _awardRank(tokenId) == 0 ? 0 : prizeAmountForRank(_awardRank(tokenId)));
    }
    ...
    function _awardRank(uint256 tokenId) private view returns (uint256) {
        return tokenId == winningTokenIds[1] ? 1 : tokenId == winningTokenIds[2] ? 2 : 0;
    }
