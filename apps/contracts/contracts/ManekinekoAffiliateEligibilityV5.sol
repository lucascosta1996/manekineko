...
contract ManekinekoAffiliateEligibilityV5 is Ownable2Step, ReentrancyGuard {
    string public constant ELIGIBILITY_VERSION = "affiliate-eligibility-v5";
    ...
    // The enrollment permit schema and round-bound EIP-712 domain remain compatible with V6-V10 rounds.
    ...
    /// @dev Existing V5/V6/V7/V8/V9 rounds can be imported as sources after successful completion and can never enroll through this gate.
    function registerCollection(address factory, uint256 id) external onlyOwner nonReentrant returns (address round) {
        ...
        bool sourceOnly = version == keccak256("affiliate-v5") || ((version == keccak256("affiliate-v6") || version == keccak256("affiliate-v7") || version == keccak256("affiliate-v8") || version == keccak256("affiliate-v9")) && _completed(candidate));
        if (candidate.roundId() != id || candidate.owner() == address(0) || candidate.maxSupply() == 0) revert InvalidCollection();
        if (sourceOnly) {
            if (keccak256(bytes(candidate.ALGORITHM_VERSION())) != (version == keccak256("affiliate-v5") ? keccak256("unique-rank-v2") : version == keccak256("affiliate-v6") ? keccak256("unique-rank-v3") : version == keccak256("affiliate-v7") ? keccak256("unique-rank-v4") : keccak256("unique-rank-v5")) || !_completed(candidate)) revert InvalidCollection();
        } else {
            bool supported = (version == keccak256("affiliate-v6") && keccak256(bytes(candidate.ALGORITHM_VERSION())) == keccak256("unique-rank-v3")) ||
                ...
                ((version == keccak256("affiliate-v8") || version == keccak256("affiliate-v9")) && keccak256(bytes(candidate.ALGORITHM_VERSION())) == keccak256("unique-rank-v5")) ||
                (version == keccak256("affiliate-v10") && keccak256(bytes(candidate.ALGORITHM_VERSION())) == keccak256("unique-rank-v6"));
            if (!supported ||
    ...
    function eligibilityStatus(address target, address applicant, address sourceCollection, uint256 sourceTokenId) public view returns (uint8) {
        ...
        bytes32 version = keccak256(bytes(targetRound.CONTRACT_VERSION()));
        bool timedEnrollment = version == keccak256("affiliate-v7") || version == keccak256("affiliate-v8") || version == keccak256("affiliate-v9") || version == keccak256("affiliate-v10");
        if ((timedEnrollment ? block.timestamp >= targetRound.saleStartAt() : targetRound.saleActivated()) || targetRound.refundsAvailable() || block.timestamp >= targetRound.mintDeadline()) return 5;
    ...
    function _completed(IAffiliateEligibilityRound round) private view returns (bool) {
        ...
        if (version == keccak256("affiliate-v7") || version == keccak256("affiliate-v8") || version == keccak256("affiliate-v9") || version == keccak256("affiliate-v10")) return round.readyForNextRound();
