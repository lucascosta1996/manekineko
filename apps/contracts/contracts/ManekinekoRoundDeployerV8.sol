...
contract ManekinekoRoundDeployerV8 {
    ...
    constructor() {
        factory = msg.sender;
        bytes memory code = type(ManekinekoRoundV8).creationCode;
        ...
        codePart1 = address(new ManekinekoV8CodePart(first));
        codePart2 = address(new ManekinekoV8CodePart(second));
        firstLength = first.length;
        secondLength = second.length;
    }
    function deploy(ManekinekoRoundV8.Config calldata config, address renderer) external returns (address round) {
