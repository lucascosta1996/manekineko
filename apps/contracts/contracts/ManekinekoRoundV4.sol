...
contract ManekinekoRoundV4 is ERC721, Ownable2Step, ReentrancyGuard, EIP712 {
    ...
    string public constant CONTRACT_VERSION = "affiliate-v4";
    bytes32 public constant ENROLLMENT_TYPEHASH = keccak256("Enrollment(address applicant,uint256 affiliateId,uint256 commissionBps,bytes32 nonce,uint256 deadline)");
    ...
    mapping(uint256 => uint256) public affiliateRateBps;
    ...
    event AffiliateEnrolled(uint256 indexed id, address indexed wallet, uint256 commissionBps, bytes32 nonce);
    ...
    constructor(Config memory config, address onchainRenderer)
        ...
        EIP712("ManekinekoAffiliateEnrollment", "2")
    {
        if (
            ...
            config.mintPrice < BPS_DENOMINATOR || config.mintPrice % BPS_DENOMINATOR != 0 ||
            config.prizeBps > BPS_DENOMINATOR || config.affiliateRatesBps.length != config.maxAffiliateSlots ||
            config.maxAffiliateSlots == 0 || config.maxAffiliateSlots > MAX_AFFILIATE_SLOTS ||
            config.enrollmentSigner == address(0) || config.enrollmentSigner == config.initialOwner || config.enrollmentSigner.code.length != 0 ||
        ...
        // Each mint has at most one affiliate; unused positions create no payment liability.
        for (uint256 i; i < config.maxAffiliateSlots; ++i) {
            uint256 rate = config.affiliateRatesBps[i];
            if (rate > BPS_DENOMINATOR || config.prizeBps + rate > BPS_DENOMINATOR) revert InvalidConfig();
            affiliateRateBps[i + 1] = rate;
        }
        maxAffiliateSlots = config.maxAffiliateSlots;
        enrollmentSigner = config.enrollmentSigner;
        renderer = ManekinekoRendererV4(onchainRenderer);
    ...
    function enrollAffiliate(address applicant, uint256 affiliateId, uint256 commissionBps, bytes32 nonce, uint256 deadline, bytes calldata signature) external nonReentrant {
        ...
        if (commissionBps != affiliateRateBps[affiliateId]) revert AffiliateRateMismatch();
        if (enrollmentNonceUsed[nonce]) revert EnrollmentNonceUsed();
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(ENROLLMENT_TYPEHASH, applicant, affiliateId, commissionBps, nonce, deadline)));
        ...
        emit AffiliateEnrolled(affiliateId, applicant, commissionBps, nonce);
    }
    ...
    /// @notice Only the selected affiliate earns its immutable rate on this paid primary mint.
    ...
    function _mintTickets(address to, uint256 quantity, uint256 affiliateId) private {
        ...
        if (affiliateId != 0) {
            uint256 commission = (mintPrice / BPS_DENOMINATOR) * affiliateRateBps[affiliateId] * quantity;
    ...
    /// @notice Owner delivers the immutable prize share of primary receipts to the winning NFT holder.
