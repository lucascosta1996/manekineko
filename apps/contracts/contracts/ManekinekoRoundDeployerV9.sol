...
contract ManekinekoRoundDeployerV9 {
    ...
    constructor() {
        factory = msg.sender;
        bytes memory code = type(ManekinekoRoundV9).creationCode;
        ...
        codePart1 = address(new ManekinekoV9CodePart(first));
        codePart2 = address(new ManekinekoV9CodePart(second));
        firstLength = first.length;
        secondLength = second.length;
    }
    function deploy(ManekinekoRoundV9.Config calldata config, address renderer) external returns (address round) {
