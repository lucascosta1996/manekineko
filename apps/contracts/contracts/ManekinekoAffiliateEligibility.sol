...
contract ManekinekoAffiliateEligibility is Ownable2Step, ReentrancyGuard {
    string public constant ELIGIBILITY_VERSION = "affiliate-eligibility-v1";
    ...
    /// @dev Existing V5 rounds can only be imported after successful completion and can never enroll through this gate.
    function registerCollection(address factory, uint256 id) external onlyOwner nonReentrant returns (address round) {
        ...
        bool sourceOnly = version == keccak256("affiliate-v5");
        if (candidate.roundId() != id || candidate.owner() == address(0) || candidate.maxSupply() == 0) revert InvalidCollection();
        if (sourceOnly) {
            if (keccak256(bytes(candidate.ALGORITHM_VERSION())) != keccak256("unique-rank-v2") || !_completed(candidate)) revert InvalidCollection();
        } else {
            if (version != keccak256("affiliate-v6") || keccak256(bytes(candidate.ALGORITHM_VERSION())) != keccak256("unique-rank-v3") ||
    ...
    function eligibilityStatus(address target, address applicant, address sourceCollection, uint256 sourceTokenId) public view returns (uint8) {
        ...
        if (targetRound.saleActivated() || targetRound.refundsAvailable() || block.timestamp >= targetRound.mintDeadline()) return 5;
