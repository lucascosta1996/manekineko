...
import {ManekinekoAffiliateEligibility} from "./ManekinekoAffiliateEligibility.sol";
...
contract ManekinekoRoundV6 is ERC721, Ownable2Step, ReentrancyGuard {
    ...
    string public constant CONTRACT_VERSION = "affiliate-v6";
    ...
    string public constant ALGORITHM_VERSION = "unique-rank-v3";
    ...
    bytes32 private constant COMBINATION_DOMAIN = keccak256("MANEKINEKO_COMBINATION_V3");
    ...
    ManekinekoAffiliateEligibility public immutable affiliateEligibility;
        ...
        ERC721(config.name, config.symbol) Ownable(config.initialOwner)
    {
        ...
        ManekinekoRendererV6(onchainRenderer).validateAppearance(config.seasonName, config.name, config.collectionColor, config.textColor);
        ManekinekoRendererV6(onchainRenderer).validateTerms(
        ...
        affiliateEligibility = ManekinekoAffiliateEligibility(config.affiliateEligibility);
        renderer = ManekinekoRendererV6(onchainRenderer);
    ...
    function combination(uint256 tokenId) public view returns (uint256[4] memory numbers, uint256 code, uint256 result) {
        ...
        result = UniqueRank.rank(tokenId, maxSupply, drawOffset);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return renderer.tokenURI(ManekinekoRendererV6.Appearance(seasonId, seasonName, name(), collectionColor(), textColor()), tokenId, refundsAvailable(), revealed, numbers, result);
    }
