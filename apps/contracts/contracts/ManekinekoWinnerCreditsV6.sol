...
/// @dev Factory approval is an explicit governance trust boundary: approve only reviewed, immutable V6/V7/V8/V9/V10 factories.
...
contract ManekinekoWinnerCreditsV6 is Ownable2Step, ReentrancyGuard {
    string public constant WINNER_CREDITS_VERSION = "winner-credits-v6";
    ...
    constructor(address initialOwner, bytes32 historicalWinnerRoot, address priorRegistry) Ownable(initialOwner) {
        ...
        if (priorRegistry != address(0)) {
            ...
            if (priorVersion != keccak256("winner-credits-v2") && priorVersion != keccak256("winner-credits-v3") && priorVersion != keccak256("winner-credits-v4") && priorVersion != keccak256("winner-credits-v5")) revert InvalidCollection();
        }
    ...
    function _multiAwards(IWinnerCreditRound round) private view returns (bool) {
        ...
        return (version == keccak256("affiliate-v7") && algorithm == keccak256("unique-rank-v4")) ||
            ((version == keccak256("affiliate-v8") || version == keccak256("affiliate-v9")) && algorithm == keccak256("unique-rank-v5")) ||
            (version == keccak256("affiliate-v10") && algorithm == keccak256("unique-rank-v6"));
    }
    ...
    function lifetimeRewardUsed(address beneficiary) public view returns (bool) {
        ...
        if (keccak256(bytes(previousRegistry.WINNER_CREDITS_VERSION())) != keccak256("winner-credits-v2"))
