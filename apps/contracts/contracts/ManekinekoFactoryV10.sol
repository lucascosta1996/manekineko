...
import {ManekinekoRoundV10} from "./ManekinekoRoundV10.sol";
import {ManekinekoRoundDeployerV10} from "./ManekinekoRoundDeployerV10.sol";
import {ManekinekoRendererV10} from "./ManekinekoRendererV10.sol";

/// @notice V10 configurable affiliate round registry. Failed/unsold rounds do not satisfy the successful-series rollover rule.
contract ManekinekoFactoryV10 is Ownable2Step {
    ManekinekoRendererV10 public immutable renderer;
    ManekinekoRoundDeployerV10 public immutable deployer;
    ...
    constructor(address initialOwner) Ownable(initialOwner) { renderer = new ManekinekoRendererV10(); deployer = new ManekinekoRoundDeployerV10(); }

    function createRound(ManekinekoRoundV10.Config calldata config) external onlyOwner returns (address round) {
        if (roundCount != 0 && !ManekinekoRoundV10(rounds[roundCount]).readyForNextRound()) revert PreviousRoundIncomplete();
