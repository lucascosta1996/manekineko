...
contract ManekinekoRoundV5 is ERC721, Ownable2Step, ReentrancyGuard, EIP712 {
    ...
    string public constant CONTRACT_VERSION = "affiliate-v5";
    bytes32 public constant ENROLLMENT_TYPEHASH = keccak256("Enrollment(address applicant,uint256 affiliateId,uint256 poolBps,bytes32 nonce,uint256 deadline)");
    ...
    constructor(Config memory config, address onchainRenderer)
        ...
        EIP712("ManekinekoAffiliateEnrollment", "3")
    {
        if (
            ...
            config.prizeBps > BPS_DENOMINATOR || config.affiliatePoolBps > BPS_DENOMINATOR ||
            config.prizeBps + config.affiliatePoolBps > BPS_DENOMINATOR ||
        ...
        renderer = ManekinekoRendererV5(onchainRenderer);
    ...
    function enrollAffiliate(address applicant, uint256 affiliateId, uint256 poolBps, bytes32 nonce, uint256 deadline, bytes calldata signature) external nonReentrant {
        ...
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(ENROLLMENT_TYPEHASH, applicant, affiliateId, poolBps, nonce, deadline)));
    ...
    /// @notice A provisional amount, never a vested balance before sellout. Later sales change shares.
    function affiliateEstimatedShare(uint256 id) external view returns (uint256) {
        if (refundsAvailable() || totalReferredMints == 0) return 0;
        if (soldOut()) return affiliateAccrued[id];
        return Math.mulDiv(affiliatePoolAmount(), affiliateReferredMints[id], totalReferredMints);
    }

    /// @dev Bounded by 100 positions and executed once at sellout. Cumulative rounding allocates
    /// the entire pool exactly, with less than one wei deviation from each proportional share.
    /// Empty positions receive zero. No referrals means no affiliate liability.
    function _allocateAffiliatePool() private {
        if (totalReferredMints == 0) return;
        uint256 pool = affiliatePoolAmount();
        uint256 cumulative;
        uint256 assigned;
        for (uint256 id = 1; id <= maxAffiliateSlots; ++id) {
            uint256 referred = affiliateReferredMints[id];
            if (referred == 0) continue;
            cumulative += referred;
            uint256 entitlement = Math.mulDiv(pool, cumulative, totalReferredMints);
            affiliateAccrued[id] = entitlement - assigned;
            assigned = entitlement;
        }
        totalAffiliateAccrued = pool;
        emit AffiliatePoolAllocated(pool, totalReferredMints);
    }
