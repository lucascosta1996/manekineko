...
contract ManekinekoV10CodePart {
...
contract ManekinekoRoundDeployerV10 {
    ...
    constructor() {
        factory = msg.sender;
        bytes memory code = type(ManekinekoRoundV10).creationCode;
        ...
        codePart1 = address(new ManekinekoV10CodePart(first));
        codePart2 = address(new ManekinekoV10CodePart(second));
        firstLength = first.length;
        secondLength = second.length;
    }
    function deploy(ManekinekoRoundV10.Config calldata config, address renderer) external returns (address round) {
