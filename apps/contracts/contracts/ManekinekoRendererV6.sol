...
contract ManekinekoRendererV6 {
    ...
    function tokenURI(Appearance calldata appearance, uint256 tokenId, bool refundable, bool revealed, uint256[4] calldata n, uint256 result)
        external pure returns (string memory)
    {
        ...
        return string.concat("data:application/json;base64,", Base64.encode(bytes(string.concat(
            ...
            '","description":"Unique ranks from 1 to collection supply. Four numbers encode each rank with a reversible VRF-keyed permutation. One highest score. ',
            'Verify the score with scoreCombination([a,b,c,d]) on this collection contract. Chainlink VRF proof verified on Ethereum. Transfers lock from sellout until prize payment.",',
            '"contract_version":"affiliate-v6","algorithm_version":"unique-rank-v3","randomness_provider":"chainlink-vrf-v2.5",',
