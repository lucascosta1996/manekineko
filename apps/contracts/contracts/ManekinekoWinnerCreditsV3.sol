...
/// @dev Factory approval is an explicit governance trust boundary: approve only reviewed, immutable V6/V7 factories.
...
contract ManekinekoWinnerCreditsV3 is Ownable2Step, ReentrancyGuard {
    string public constant WINNER_CREDITS_VERSION = "winner-credits-v3";
    ...
    function registerCollection(address factory, uint256 id) external nonReentrant returns (address round) {
        ...
        if (v7 && (candidate.awardCount() == 0 || candidate.awardCount() > 2)) revert InvalidCollection();
    ...
    function _isV7(IWinnerCreditRound round) private view returns (bool) {
        return keccak256(bytes(round.CONTRACT_VERSION())) == keccak256("affiliate-v7") &&
            keccak256(bytes(round.ALGORITHM_VERSION())) == keccak256("unique-rank-v4");
    }
    function _credit(address sourceRound, uint256 rank) private view returns (Credit storage credit) {
        if (rank == 0 || rank > 2) revert CreditUnavailable();
    ...
    function lifetimeRewardUsed(address beneficiary) public view returns (bool) {
        return redeemedSource[beneficiary] != address(0) ||
            (address(previousRegistry) != address(0) && previousRegistry.redeemedSource(beneficiary) != address(0));
    }
