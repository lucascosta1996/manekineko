...
import {ManekinekoRendererV9} from "./ManekinekoRendererV9.sol";
...
contract ManekinekoRoundV9 is ERC721, Ownable2Step, ReentrancyGuard {
    ...
    string public constant CONTRACT_VERSION = "affiliate-v9";
    ...
    ManekinekoRendererV9 public immutable renderer;
        ...
        ERC721(config.name, config.symbol) Ownable(config.initialOwner)
    {
        ...
        ManekinekoRendererV9(onchainRenderer).validateAppearance(config.seasonName, config.name, config.collectionColor, config.textColor);
        ManekinekoRendererV9(onchainRenderer).validateTerms(
        ...
        renderer = ManekinekoRendererV9(onchainRenderer);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return renderer.tokenURI(ManekinekoRendererV9.Appearance(seasonId, seasonName, name(), collectionColor(), textColor()), tokenId, refundsAvailable(), revealed, numbers, result, _awardRank(tokenId), _awardRank(tokenId) == 0 ? 0 : prizeAmountForRank(_awardRank(tokenId)));
    }
