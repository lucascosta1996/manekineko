...
import {ManekinekoRoundV9} from "./ManekinekoRoundV9.sol";
import {ManekinekoRoundDeployerV9} from "./ManekinekoRoundDeployerV9.sol";
import {ManekinekoRendererV9} from "./ManekinekoRendererV9.sol";

/// @notice V9 configurable affiliate round registry. Failed/unsold rounds do not satisfy the successful-series rollover rule.
contract ManekinekoFactoryV9 is Ownable2Step {
    ManekinekoRendererV9 public immutable renderer;
    ManekinekoRoundDeployerV9 public immutable deployer;
    ...
    constructor(address initialOwner) Ownable(initialOwner) { renderer = new ManekinekoRendererV9(); deployer = new ManekinekoRoundDeployerV9(); }

    function createRound(ManekinekoRoundV9.Config calldata config) external onlyOwner returns (address round) {
        if (roundCount != 0 && !ManekinekoRoundV9(rounds[roundCount]).readyForNextRound()) revert PreviousRoundIncomplete();
