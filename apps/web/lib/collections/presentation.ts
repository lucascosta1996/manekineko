...
export function collectionProgress(collection: Pick<CollectionPublic, "phase" | "contractVersion" | "saleStartAt">, now = Date.now()): { label: string; detail: string } {
  ...
  switch (collection.phase) {
    ...
    case "awaiting_randomness": return { label: "Awaiting randomness", detail: "The collection is sold out and waiting for Chainlink VRF." };
    case "awaiting_reveal": return { label: "Awaiting reveal", detail: "The collection is sold out. The winning result has not been revealed yet." };
    ...
    case "awaiting_prize": return { label: ranked ? "Prizes available" : "Prize pending", detail: ranked ? "The winning tickets are finalized. Each unpaid prize remains reserved for its NFT holder to claim." : "The winning result is finalized. The prize has not been paid yet." };
    ...
    case "refundable": return { label: "Refunds available", detail: "The collection ended without a prize. Eligible holders can claim their ticket refunds." };
