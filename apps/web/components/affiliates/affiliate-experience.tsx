...
import { formatRewardAllocation } from "./program-terms";
...
const states: Record<AffiliateStatus, { label: string; title: string; description: string; step: number }> = {
  ...
  no_commission: { label: "Referrals recorded", title: "Your referrals are recorded. No reward is due.", description: "Successful purchases used your link, but this position does not earn rewards. Your referred tickets are recorded without generating an unpaid balance.", step: 3 },
  no_referrals: { label: "Ready to share", title: "Your link is ready. Your first referral is next.", description: "No successful NFT purchases have used your link yet, so no rewards have accrued. A click alone does not generate earnings.", step: 1 },
  pending_sellout: { label: "Pending sellout", title: "Your referrals are recorded.", description: "Purchases through your link have earned pending rewards. They become available to claim when this collection sells out.", step: 2 },
  claimable: { label: "Available to claim", title: "The collection sold out. Your earnings are ready.", description: "Claim your available rewards to your wallet or another receiving address. You do not need to wait for prize payments to winners.", step: 3 },
  paid: { label: "Paid", title: "Your rewards have been claimed.", description: "Your earned rewards have been claimed. The payment stays recorded for this collection, even after the next collection opens.", step: 4 },
  refunded: { label: "Refund period", title: "This collection closed before sellout.", description: "Pending rewards were cancelled so NFT holders can receive their full mint-price refunds. There is no affiliate payment due for this collection.", step: 2 },
};
const scenarioLabels: Record<DemoScenario, string> = { no_commission: "No rewards", no_referrals: "No referrals", pending_sellout: "Pending sellout", claimable: "Claimable", paid: "Paid", refunded: "Refund period" };
...
export function AffiliateExperience({ collection: initialCollection }: { collection: CollectionPublic }) {
  ...
  async function enroll() {
    await action(async () => {
      ...
      if (permit.contractVersion !== program.contractVersion || (auth.offer && (permit.affiliateId !== auth.offer.affiliateId || permit.commissionBps !== auth.offer.commissionBps))) throw new Error("The enrollment authorization does not match the position and reward terms you reviewed.");
  ...
  async function claim() {
    await action(async () => {
      if (!wallet || !program?.readiness.canClaim || !program.account || BigInt(program.account.claimableWei) <= 0n) throw new Error("There are no rewards available to claim.");
      ...
      setJournal(null); setMessage("Your affiliate rewards were paid. The balance has been refreshed."); await refresh();
    });
  ...
  const equalTerms = `Qualify with at least ${program?.minAffiliateReferrals ?? "—"} paid referrals. Qualified affiliates earn equal rewards, subject to the collection’s payout limit. Your final amount is fixed at sellout. The full budget is not guaranteed to be distributed.`;
  ...
  const baseStatus = accountView.state === "disconnected" ? { label: "Wallet not connected", title: "See your affiliate position.", description: "Connect the wallet you enrolled with to see its referrals, payments and available rewards.", step: 0 }
    : accountView.state === "loading" ? { label: "Checking wallet", title: "Reading your on-chain position.", description: "Checking this wallet’s enrollment, referrals and rewards. No transaction is needed.", step: 0 }
    : accountView.state === "unavailable" ? { label: "Rewards unavailable", title: "Rewards could not be verified.", description: "Refresh on-chain data to try again. Missing data does not mean you are unregistered or have a zero balance.", step: 0 }
  ...
  const status = equalPool && account?.affiliateId && !program?.soldOut && !program?.refundable ? { ...baseStatus, description: account.qualified ? "You meet the referral minimum. Your equal share stays provisional until sellout; it can change as other affiliates qualify." : `You need ${account.referralsRemaining} more paid referrals to qualify for the equal split. Enrollment alone earns no rewards.` } : pool && account?.status === "pending_sellout" ? { ...baseStatus, description: "Your estimate follows your share of all referred sales. It can rise or fall until sellout, when your final pool allocation becomes claimable." } : pool && account?.status === "no_commission" ? { ...baseStatus, description: "Your referred tickets are recorded. No reward is available to claim. The contract fixes eligible rewards at sellout." } : baseStatus;
  ...
  const selloutRevenue = program ? BigInt(program.mintPriceWei) * BigInt(program.maxSupply) : 0n;
  ...
  const enrollmentTerms = (rate: number | undefined) => {
    if (!program || rate === undefined) return "Reward terms are unavailable. Refresh before enrolling.";
    if (pool) return `Affiliate reward budget at sellout: up to ${formatRewardAllocation(selloutRevenue, rate)} ETH. ${equalPool ? equalTerms : "Your reward depends on your share of paid referrals and is finalized at sellout."}`;
    return `Reward per paid referral: ${formatRewardAllocation(program.mintPriceWei, rate)} ETH. This amount is fixed for this collection.`;
  };
  ...
  return <div className="affiliate-experience">
    <section className="affiliate-hero" aria-labelledby="affiliate-title">
      <div>
        ...
        <h1 id="affiliate-title">Share your link.<br /><span>Earn ETH rewards.</span></h1>
        <p>Refer paid mints to {collection.name}, meet the referral requirement, and claim your earned rewards directly from the contract after sellout.</p>
      </div>
    ...
    {program && <>
      ...
      <section className="affiliate-dashboard" aria-label="Your affiliate position" aria-busy={loading}>
        <div className="affiliate-position-card">
          ...
          <div className="affiliate-journey" aria-label="Affiliate reward lifecycle">{(account?.status === "refunded" ? ["Enroll", "Share", "Closed unsold", "No payment"] : account?.status === "no_commission" ? ["Enroll", "Share", "Referrals", "No payment"] : ["Enroll", "Share", "Sell out", "Claim"]).map((step, i) => <div key={step} className={status.step > i ? "is-complete" : ""}><span>{status.step > i ? "✓" : `0${i + 1}`}</span><small>{step}</small></div>)}</div>
          {account?.referralUrl && <div className="affiliate-share">
            ...
            <div><input id="affiliate-link" value={account.referralUrl} readOnly onFocus={(event) => event.currentTarget.select()} /><button type="button" onClick={() => void copyLink()}>{copied ? "Copied ✓" : "Copy referral link"}</button></div>
            <p>{demo ? "This link opens a demo mint. It cannot create real rewards." : "The contract fixes your beneficiary wallet. Each successful referred mint is counted once."}</p>
          </div>}
          ...
          {!demo && <div className="affiliate-wallet-actions">
            ...
            {program.enrollmentEligibility && <div className="affiliate-eligibility">
              ...
              {wallet && account?.status==="unregistered" && program.enrollmentEligibility.policy==="nft_holder" && !auth && <>
                ...
                {program.enrollmentEligibility.tokens.length===0&&<p>No completed-collection NFTs were found on this page for this wallet. <Link className="text-link" href="/my-nfts">View my tickets →</Link> If you just received one, wait for confirmation and refresh. Ownership is checked again when enrollment executes.</p>}
            ...
            {wallet && account?.status === "unregistered" && program.enrollmentStatus === "open" && !auth && <><p className="affiliate-enrollment-review">{program.enrollmentOffer ? <>Offered position #{program.enrollmentOffer.affiliateId}: {enrollmentTerms(program.enrollmentOffer.commissionBps)}</> : "No affiliate position is currently offered."}</p><button className="primary-button" disabled={busy || loading || !program.readiness.canEnroll || !nftSelectionReady || !!journal} onClick={() => void prepareEnrollment()}><span>{busy ? "Waiting for wallet…" : "Join affiliate program"}</span><span aria-hidden="true">↗</span></button></>}
            {auth && program.turnstileSiteKey && <><p className="affiliate-enrollment-review">Enrollment terms{auth.offer ? ` · position #${auth.offer.affiliateId}` : ""}: {enrollmentTerms(enrollmentRate)} {enrollmentRate === 0 ? "This position records referrals but does not earn rewards." : "Rewards become available at sellout; it is cancelled if the collection expires unsold."}</p><TurnstileCheck siteKey={program.turnstileSiteKey} challengeId={auth.challenge.challengeId} onToken={setToken} /><button className="primary-button" onClick={() => void enroll()} disabled={!token || busy || !!journal}><span>{busy ? "Registering your position…" : "Confirm on-chain enrollment"}</span><span aria-hidden="true">↗</span></button><p>Enrollment requires an Ethereum network fee. Your position is assigned only when the transaction succeeds.</p></>}
          </div>}
          {program.readiness.reason && <p className="affiliate-readiness">{program.readiness.reason}</p>}
        </div>
        <div className="affiliate-balance-card">
          <div className="affiliate-card-heading"><span className="eyebrow">{demo ? "EXAMPLE AFFILIATE REWARDS" : "YOUR AFFILIATE REWARDS"}</span><span aria-hidden="true">↗</span></div>
          <span className="affiliate-balance-label">Available to claim</span><p className="affiliate-main-balance">{eth(account?.claimableWei)} <small>ETH</small></p>
          <dl className="affiliate-balances"><div><dt>{program.soldOut ? "Final affiliate reward" : pool ? "Estimated affiliate reward" : "Pending reward"}</dt><dd>{eth(program.soldOut ? account?.accruedWei : account?.pendingWei)} ETH</dd></div><div><dt>Rewards claimed</dt><dd>{eth(account?.claimedWei)} ETH</dd></div><div><dt>Referred tickets</dt><dd>{accountView.state === "ready" ? formatCount(account?.referredMints ?? 0) : "—"}</dd></div>{pool && !equalPool && <div><dt>Referred ticket value</dt><dd>{accountView.state === "ready" ? formatWei(BigInt(account?.referredMints ?? 0) * BigInt(program.mintPriceWei)) : "—"} ETH</dd></div>}{equalPool && <><div><dt>Qualification</dt><dd>{accountView.state !== "ready" ? "—" : account?.qualified ? "Qualified" : `${account?.referredMints ?? 0} / ${program.minAffiliateReferrals} referrals`}</dd></div><div><dt>Qualified affiliates</dt><dd>{program.qualifiedSlots} / {program.enrolledSlots} enrolled</dd></div><div><dt>{program.soldOut ? "Reward per qualified affiliate" : "Estimated reward per qualifier"}</dt><dd>{formatWei(program.equalShareWei ?? "0")} ETH</dd></div><div><dt>Unfilled positions</dt><dd>{program.availableSlots}</dd></div></>}</dl>
          {!demo && account?.status === "claimable" && <div className="affiliate-recipient"><label htmlFor="affiliate-recipient">Receiving address</label><input id="affiliate-recipient" type="text" value={recipient} onChange={(event) => setRecipient(event.target.value.trim())} placeholder="0x…" autoComplete="off" spellCheck={false} disabled={busy} /><p>Your registered wallet authorizes the claim. The contract sends ETH directly to this address. No manual payout approval.</p></div>}
          <button className="primary-button" onClick={() => void claim()} disabled={demo || !wallet || busy || accountView.state !== "ready" || !!journal || !program.readiness.canClaim || BigInt(account?.claimableWei ?? "0") === 0n}><span>{busy ? "Check your wallet…" : demo ? "Preview only · no claim" : account?.status === "paid" ? "Rewards claimed" : "Claim affiliate rewards"}</span><span aria-hidden="true">↗</span></button>
          <p className="affiliate-claim-note">{accountView.state === "disconnected" ? "Connect your registered wallet to check its rewards. Reading balances is free." : accountView.state !== "ready" ? "Your balance is not verified yet. Refresh on-chain data before attempting a withdrawal." : account?.status === "paid" ? "All rewards earned by this position have already been claimed. The total appears under Rewards claimed." : account?.status === "unregistered" ? "This wallet has no affiliate position in this collection." : account?.status === "no_commission" ? "Your referrals are recorded. This position has no reward available to claim." : account?.status === "no_referrals" ? program.soldOut || program.refundable ? "No purchases were attributed to this position. There are no rewards to claim from this closed collection." : "No referrals yet means no unpaid rewards. Share your link to start." : account?.status === "refunded" ? "Full NFT refunds take priority. Pending affiliate earnings were cancelled." : "Earned rewards become claimable at sellout. The contract pays your chosen address directly; a network fee applies."}</p>
        </div>
      </section>
      <details className="affiliate-program-terms">
        <summary>Program terms <span aria-hidden="true">+</span></summary>
        <dl><div><dt>{pool ? "Affiliate reward budget at sellout" : "Reward per paid referral"}</dt><dd>{commissionBps === undefined ? "Reward terms unavailable" : `${pool ? "Up to " : ""}${formatRewardAllocation(pool ? selloutRevenue : program.mintPriceWei, commissionBps)} ETH`}</dd></div><div><dt>Prizes at sellout</dt><dd>{formatRewardAllocation(selloutRevenue, program.prizeBps)} ETH</dd></div><div><dt>Payment availability</dt><dd>{commissionBps === 0 ? "No rewards for this position" : equalPool ? `At sellout, after at least ${program.minAffiliateReferrals} paid referrals` : "At sellout, after a successful referred mint"}</dd></div><div><dt>If the sale expires unsold</dt><dd>Pending rewards are cancelled; NFT holders keep full refund rights</dd></div></dl>
        {equalPool ? <p>{equalTerms} The shared payout limit is {formatRewardAllocation(program.mintPriceWei, program.affiliatePayoutCapBps ?? 0)} ETH multiplied by the fewest paid referrals among qualified affiliates. Unallocated funds remain in the separate growth reserve. Estimates may rise or fall before sellout. Referrals credited to another position cannot count toward your minimum.</p> : pool ? <p>The pool is funded from primary mint payments. Your final payout is the pool multiplied by your referred sales divided by all affiliates’ referred sales. Shares change until sellout. No referrals means no earnings. If one affiliate brings all referred sales, they receive the whole pool. If nobody refers a sale, the unused pool stays with the operator. Increasing the number of positions does not enlarge the pool.</p> : <>
        <p>The buyer pays the advertised ticket price. Affiliate rewards come from the collection’s operating allocation and do not reduce its configured prize. Each affiliate earns only from their own referrals. Each position’s reward per paid referral is fixed for this collection; unused positions do not increase another position’s earnings.</p><div className="affiliate-rate-schedule"><h3>Reward per paid referral by position</h3><ol>{program.affiliateRatesBps.map((rate, index) => <li key={index}><span>#{String(index + 1).padStart(2, "0")}</span><strong>{formatRewardAllocation(program.mintPriceWei, rate)} ETH</strong></li>)}</ol></div></>}
    ...
    <section className="affiliate-explainer" aria-labelledby="affiliate-how-title">
      <div className="section-heading"><div><p className="eyebrow">THE REFERRAL PROGRAM</p><h2 id="affiliate-how-title">Rewards enforced by the contract.</h2></div><Link className="text-link" href={`/mint/${collection.id}/contract#affiliate-rules`}>Read the contract <span aria-hidden="true">↗</span></Link></div>
      <div className="how-grid"><article><span className="step-label">01 / JOIN</span><h3>{holderProgram && !bootstrap ? "Check your affiliate eligibility." : "Join the affiliate program."}</h3><p>{enrollmentCopy.join}</p></article><article><span className="step-label">02 / SHARE</span><h3>Refer paid mints.</h3><p>Share your collection link. Buyers see the registered beneficiary and payment terms before signing their mint. Their ticket price stays the same. Clicks and NFT ownership alone do not generate earnings.</p></article><article><span className="step-label">03 / EARN</span><h3>Claim rewards after sellout.</h3><p>{equalPool ? equalTerms : pool ? "Your successful referrals determine your share of the affiliate pool. Estimates can change until sellout, when the final allocation becomes withdrawable." : "Successful purchases add pending rewards that become claimable at sellout."} No successful referrals means no earnings. If the sale expires unsold, there is no affiliate payout.</p></article></div>
      <div className="affiliate-trust-note"><span aria-hidden="true">◇</span><p>{holderProgram ? "The smart contracts check NFT ownership and prevent the same NFT from unlocking multiple positions in one collection. " : "This collection follows its original admission rules. "}Earned affiliate rewards stay protected in the contract until claimed. The contract pays your chosen receiving address directly. Automated checks reduce abuse, but wallets and IP addresses do not prove unique people. Each new collection requires its own enrollment and keeps separate earnings.</p></div>
    </section>
