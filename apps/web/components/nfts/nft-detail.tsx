...
export function NftDetail({ item }: { item: NftItem }) {
  ...
  return <div className="nft-page nft-detail-page">
    <nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/my-nfts">My NFTs</Link><span aria-hidden="true">/</span><Link href={`/mint/${nft.collectionId}`}>Collection {roundLabel(nft.roundId)}</Link><span aria-hidden="true">/</span><span aria-current="page">Ticket #{nft.tokenId}</span></nav>
    <div className="nft-detail-grid">
      ...
      <section className="nft-detail-information" aria-labelledby="nft-title"><div className="nft-detail-badges"><span className="nft-status">{nft.networkName}</span><span className="nft-status nft-status-complete">{collectionProgress(nft).label}</span>{nft.winningToken && <span className="nft-winner-badge">✦ Winning ticket</span>}</div><p className="eyebrow">COLOR. RECORDED ON-CHAIN.</p><h1 id="nft-title">Ticket #{nft.tokenId}</h1><Link className="nft-detail-collection" href={`/mint/${nft.collectionId}`}>{nft.name} <span aria-hidden="true">↗</span></Link>
        ...
        <p className="nft-marketplace-note">{permanent ? "This NFT’s numbers and artwork stay fixed through the draw and claims. Explorers display that permanent metadata; scores and prize status are verified separately on this page. Initial explorer indexing may take time." : nft.chainId === 11155111 ? "View this ticket’s artwork and traits on Blockscout, or its token record on Etherscan. This collection’s external previews may take longer to refresh after reveal; this page reads the contract directly." : "This collection’s marketplace artwork can take longer to refresh after reveal. The preview here comes directly from the NFT contract."} <Link href="/docs/my-nfts">Understand artwork and draw results →</Link></p>
      </section>
    </div>
    <section className="nft-provenance" aria-labelledby="nft-provenance-title"><div><p className="eyebrow">THE RECORD BEHIND YOUR TICKET</p><h2 id="nft-provenance-title">Yours to verify.</h2><p>Mint history stays with a ticket. Ownership can change after the collection allows transfers.</p></div><dl>
