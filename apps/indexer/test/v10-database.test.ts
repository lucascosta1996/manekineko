...
test('V10 six equal awards settle independently and preserve refund and reorganization guarantees',{
 ...
 const schema='manekineko_v10_indexer_'+randomUUID().replaceAll('-','');const admin=new pg.Client({connectionString});await admin.connect();let pool:pg.Pool|undefined;
 try{
  ...
  for(const name of (await readdir(folder)).filter(n=>n.endsWith('.sql')&&!n.startsWith('023')).sort())await pool.query(await readFile(new URL(name,folder),'utf8'));
  const c:RegisteredCollection={id:randomUUID(),seriesId:randomUUID(),chainId:1,roundId:'1',name:'V10',symbol:'V10',maxSupply:1000,mintPrice:'10000000000000000',prizeBps:6000,winnerCount:6,affiliatePoolBps:2000,minAffiliateReferrals:100,affiliatePayoutCapBps:3000,saleStartAt:1800350000,address:wallet,factory,deploymentTransaction:hash(1),deploymentBlock:10,deployedAt:new Date(1800349900*1000).toISOString(),mintDeadline:1800436400,maxAffiliateSlots:10,enrollmentSigner:wallet,contractVersion:'affiliate-v10',algorithmVersion:'unique-rank-v6'};
  await pool.query('INSERT INTO manekineko_series(id,name) VALUES($1,$2)',[c.seriesId,'Test']);
  await pool.query(`INSERT INTO manekineko_collections(id,series_id,chain_id,round_id,slug,name,symbol,max_supply,mint_price_wei,mint_duration_seconds,reveal_delay_blocks,algorithm_version,randomness_provider,contract_version,prize_bps,affiliate_pool_bps,winner_count,min_affiliate_referrals,affiliate_payout_cap_bps,sale_start_at,season_id,season_name,collection_color,text_color)
   VALUES($1::uuid,$2,1,1,$1::text,'V10','V10',1000,10000000000000000,86400,NULL,'unique-rank-v6','chainlink-vrf-v2.5','affiliate-v10',6000,2000,6,100,3000,to_timestamp($3),$4,'V10 season','#330000','#FFFFFF')`,[c.id,c.seriesId,c.saleStartAt,key]);
  ...
  await pool.query(`INSERT INTO manekineko_affiliate_programs(collection_id,mode,contract_version,max_slots,commission_bps,affiliate_rates_bps,enrollment_signer) VALUES($1,'live','affiliate-v10',10,NULL,'{}',$2)`,[c.id,wallet]);
  await assert.rejects(pool.query('UPDATE manekineko_collections SET winner_count=5 WHERE id=$1',[c.id]),/immutable/);
  const payload={contract:{algorithmVersion:'unique-rank-v6',maxMintsPerWallet:'20',winnerCount:'6',prizeBps:'6000',maxSupply:'1000',affiliatePoolBps:'2000',minAffiliateReferrals:'100',affiliatePayoutCapBps:'3000',saleStartAt:'0'}};
  assert.equal((await pool.query('SELECT manekineko_launch_payload_version($1::jsonb) version',[JSON.stringify(payload)])).rows[0].version,'affiliate-v10');
  ...
  const makeAward=(rank:number,claimed=false):AwardSnapshot=>{const combination=encodePermanentCombination(320+rank,key);return{rank,tokenId:320+rank,score:String(1001-rank),amountWei:'1000000000000000000',claimed,holder:wallet,winningHolder:claimed?wallet:null,paidAt:claimed?1800350300+rank:null,numbers:combination.numbers,code:combination.combinationCode,key};};
