...
Tincta should feel like a colorful collectible brand with a precisely explained prize mechanic. Use warm paper, ink-colored typography, large tightly set headlines, and the existing seasonal palette. Give the opening proposition room, then present prize and commission figures in a separate section below the hero, with units, per-collection or catalog scope, and draft/full-sellout context visibly attached. Numbers must come from the repository's current terms or authoritative runtime data; reference-site rates and metrics have no role in Tincta copy.

Use a spacious hero with quiet animated season-color SVG geometry, one headline, one supporting sentence and one rewards action. Artwork should read as a considered composition rather than confetti. Keep detailed explanations, totals and email registration outside the hero so the text and action area remains visually calm and high contrast.

Below the hero, start with the planned catalog reward totals and their conditions, followed by a centered email signup. Continue with the contract-controlled reward explanation and the deeper collection story:
...
- Planned reward amounts and labels remain legible without animation, on mobile, and before client hydration.
...
The redesign lives in `apps/landing-page`, with a server page that passes only season numbers, color arrays and a public planned-reward summary into `app/landing-experience.tsx`. Unpublished theme and collection names are excluded. The hero reads **“Autonomous rewards. Verifiable onchain.”**, followed by the single sentence **“ETH prizes and affiliate rewards, governed by smart contracts.”** and one **“Explore the rewards”** action. The animated sculpture retains a pause control; the hero has no launch kicker, separate verification paragraph, email form or palette selectors.
...
The page includes the 22-season color browser, six permanent SVG NFT previews, six equal prize illustrations, affiliate explanation, three-step introduction and expandable FAQs. Native scrolling remains intact. The hero cycles six representative season palettes, users can pause motion, and device reduced-motion preferences disable animation. Palette selection belongs to the season browser farther down the page. All image and icon assets are SVG; no raster or third-party artwork is used. The visual layer adds no runtime packages; signup uses the repository’s existing `pg` driver and `server-only` boundary.
...
Staging setup on 2026-09-23 UTC: the complete repository check passed (tests, typechecks and builds), brand/artwork generation checks passed, and both newsletter PostgreSQL 17 suites passed with no skips in a disposable local container. The pinned staging migration 028, ledger checksum and dedicated newsletter login/grants were then applied and verified. Landing now has the protected `manekineko-staging-landing` Vercel project, linked to `main`, with its canonical staging origin, staging Web link and encrypted server-only newsletter settings. Normal Git pushes deploy it alongside Web, Launch and Indexer. Hosted signup persistence is a separate check; no test email registrations were inserted into staging by this setup.

## Contract-focused copy refresh — 2026-09-23

The landing headline is now “Autonomous rewards. Verifiable onchain.” All marketing copy leads with ETH prize values, contract-calculated commissions, direct recipient claims and public verification. Color remains a visual identity element rather than the marketing message; percentage labels are removed from the page and FAQ.

The copy distinguishes protected unpaid prizes and earned affiliate commissions from unallocated affiliate reserve, which the operator can withdraw after reveal. Recipients submit claims; autonomy does not imply automatic wallet transfers or an entirely unattended launch. Contract/source verification links are described as forthcoming until V10 deployment and explorer source verification are complete. Planned catalog totals retain sellout, qualification and launch-pending conditions.

Hero simplification validation on 2026-09-24 UTC: the landing production build (including TypeScript) and 13 policy/catalog tests passed; the two disposable-database integration suites were skipped. Local browser review covered 320px/390px phones, 768px tablet and desktop, with no horizontal overflow at the checked phone/tablet widths. Signup and reward anchors, keyboard focus into the email field, and animation pause/resume worked. No signup was submitted, and these checks do not establish hosted deployment or database persistence for this revision.
