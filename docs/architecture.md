...
| `apps/landing-page` | Independently deployable public landing app; contract-focused reward copy, all-season planned ETH rewards, collection explorer, NFT previews and email registration | Separate from Web and the private console. Planned catalog budgets are 1,296 ETH prizes / up to 292 ETH affiliate pools, not live balances. Email signup uses its own restricted database login and migration 028; see [landing design research](landing-design-research.md). |
...
**Git deployment setup (updated 2026-09-23):** the Web, Launch, Indexer and Landing staging projects are connected to [lucascosta1996/manekineko](https://github.com/lucascosta1996/manekineko), with `main` as their production branch. Normal application releases use pushes to `main`; the app configurations disable automatic deployments from other branches. Environment/protection settings and Indexer schedules remain in Vercel. Database migrations, contracts and season execution remain separate. See [Git deployment workflow](git-deployments.md). Landing was added on 2026-09-23 with protection on all deployments, its own encrypted newsletter environment and the staging Web URL. Newsletter migration 028 and the dedicated restricted runtime login were applied and verified; no fixture registrations were inserted.
...
Public Web copy leads with ETH reward values, protected funds and contract-enforced claims. Navigation uses **Seasons / Results / My Tickets / How It Works**, retaining existing URLs. Display financial terms as exact native-currency amounts rather than percentage labels, including affiliate enrollment reviews and historical per-referral rewards. Distinguish configured **Prizes at sellout** and affiliate budgets from provisional estimates, claimable balances and confirmed payments. Current protected reward payouts go directly from the contract when the entitled wallet claims; do not describe them as automatic transfers or imply that operating funds, unallocated reserves and protocol execution are outside operator control. Collection artwork, contract rules, historical versions and pending rollout notices remain authoritative.
